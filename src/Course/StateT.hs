...
instance Bind f => Apply (StateT s f) where
  (<*>) ::
    ...
    -> StateT s f a
    -> StateT s f b
  (StateT fab) <*> (StateT sa) = 
      StateT $ \s -> do (f, s1) <- fab s
                        first f <$> sa s1
...
distinctF ::
  ...
  List a
  -> Optional (List a)
distinctF = (`evalT` S.empty) . filtering go 
  where go a = do
          if (a > 100) then 
            StateT (const Empty)
          else do 
