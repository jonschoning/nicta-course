...
findLeft p (ListZipper l a r) =
  case break p l of
    (_, Nil) -> IsNotZ
    (ll, lrh :. lrt) -> IsZ $ 
      ListZipper lrt lrh $ (reverse ll) ++ (a:.r)
...
findRight p (ListZipper l a r) =
  case break p r of
    (_, Nil) -> IsNotZ
    (rl, rrh :. rrt) -> IsZ $ 
      ListZipper ((reverse rl) ++ (a:.l)) rrh rrt
...
moveLeft (ListZipper (lh:.lt) a r) = IsZ $
  ListZipper lt lh (a:.r)
...
moveRight (ListZipper l a (rh:.rt)) = IsZ $
  ListZipper (a:.l) rh rt
...
swapLeft (ListZipper (lh:.lt) a r) = IsZ $ ListZipper (a:.lt) lh r
...
swapRight (ListZipper l a (rh:.rt)) = IsZ $ ListZipper l rh (a:.rt)
...
moveLeftN 0 a = IsZ a
moveLeftN n a = moveLeftN (pred n) -<< moveLeft a 
...
moveRightN 0 a = IsZ a
moveRightN n a = moveRightN (pred n) -<< moveRight a
...
moveLeftN' n a | n < 0 = moveRightN' (-n) a
moveLeftN' 0 a = Right a
moveLeftN' n a = let len = length (lefts a) 
                 in if n > len then Left len else (
                      case moveLeftN n a of 
                        IsZ x -> Right x
                        _ -> error "moveLeftN' bug")
...
moveRightN' ::
  ...
  -> Either Int (ListZipper a)
moveRightN' n a | n < 0 = moveLeftN' (-n) a
moveRightN' 0 a = Right a
moveRightN' n a = let len = length (rights a) 
                  in if n > len then Left len else (
                       case moveRightN n a of 
                         IsZ x -> Right x
                         _ -> error "moveRightN' bug")
...
nth ::
  ...
  -> MaybeListZipper a
nth i _ | i < 0 = IsNotZ
nth i a = case moveLeftN' i a of
   Left n -> moveRightN (i-n) a
   Right x -> moveLeftN (length $ lefts x) a
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft x (ListZipper l a r) = 
  ListZipper (a:.l) x r
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight x (ListZipper l a r) =
  ListZipper l x (a:.r)
...
instance Apply ListZipper where
   ListZipper la f ra  <*>  ListZipper lb a rb  =
      ListZipper (zipWith id la lb) (f a) (zipWith id ra rb)
...
instance Extend ListZipper where
  f <<= a = 
    ListZipper (unfoldr (go moveLeft) a) (f a) (unfoldr (go moveRight) a)
    where go move x = (f &&& id) <$> (toOptional $ move $ x)
