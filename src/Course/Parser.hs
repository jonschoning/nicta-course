...
mapParser ::
  ...
  -> Parser a
  -> Parser b
mapParser f (P pa) = P $ \i -> case pa i of
...
bindParser ::
  ...
  -> Parser a
  -> Parser b
bindParser f (P pa) = P $ \i -> case pa i of 
   Result i' a -> f a `parse` i'
   ErrorResult e -> ErrorResult e
...
-- | Return a parser that prepends first parser to second parsers results
consP :: Parser a -> Parser (List a) -> Parser (List a)
consP = lift2 (:.)
...
natural ::
  Parser Int
natural =
  list1 digit >>= \a -> case read a of
    Full d -> valueParser d
    Empty -> failed
...
sequenceParser ::
  List (Parser a)
  -> Parser (List a)
sequenceParser = foldRight consP (valueParser Nil)
...
firstNameParser ::
  Parser Chars
firstNameParser = upper `consP` list lower
...
surnameParser ::
  Parser Chars
surnameParser =
  upper `consP` lift2 (++) (thisMany 5 lower) (list lower)
...
phoneParser ::
  Parser Chars
phoneParser =
  digit `consP` phoneBodyParser <* is '#'
