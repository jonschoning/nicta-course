...
hex ::
  Parser Char
hex =
  thisMany 4 (satisfy isHexDigit) >>= \hs -> 
    (return . chr <$> readHex hs) ?? failed
...
eof = P $ \case
  Nil    -> Result Nil ()
  (x:._) -> ErrorResult (UnexpectedChar x)  
...
betweenSepbyComma o c pa =
  betweenCharTok o c (sepby pa (spaces *> charTok ','))
